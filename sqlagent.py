...
from langchain.agents import OutputFixingParser, RetryOutputParser
    ...
    with st.chat_message("assistant"):
        with st.spinner("Thinking..."):
            try:
                ...
                # Parsing error handling: retry or fix parsing if the output is malformed
                output_parser = OutputFixingParser(
                    retry_parser=RetryOutputParser()
                )

                # Attempt to parse the response using the output parser
                fixed_response = output_parser.parse(response)

                # Ensure that the response contains the final answer (not action)
                if "final_answer" in fixed_response:
                    message = fixed_response["final_answer"]
                    st.session_state.messages.append({"role": "assistant", "content": message})
                    st.chat_message("assistant").markdown(message)

                # Handle intermediate steps if present (SQL query or action steps)
                if "intermediate_steps" in fixed_response and fixed_response["intermediate_steps"]:
                    for step in fixed_response["intermediate_steps"]:
                        if isinstance(step, tuple):
                            st.chat_message("assistant").markdown(step[0])
                        else:
